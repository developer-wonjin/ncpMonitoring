...
<html>
	...
	<body>
		<div id="form" class="form-inline">
			...
			<button type="submit" id="submit" class="btn btn-secondary" onclick="clickSubmitInSearch()">제출</button><br>
		</div>
		<a href="/">뒤로가기</a>
...
// 	            cardHeader.innerHTML = '<span>' + serverName + ' (' + cntServerCPU +'C, ' + volumeServerMemory + 'GB)</span>&nbsp;<a href="#" class="copy" style="" data-idx="'+ num +'" data-toggle="tooltip" data-placement="top" title="" data-original-title="Copy to Clipboard">copy</a>';
	            ...
	            var fileSystemCriteria = 80;
	            var isOverFileSystem = (serverList[num]['xvd'][0] >= fileSystemCriteria) ||
									   (serverList[num]['xvd'][1] >= fileSystemCriteria) || 
									   (serverList[num]['xvd'][2] >= fileSystemCriteria)
	            
	            var innerString =    '<th id="" class="' + isOverFileSystem + '">dev/xvda1, b1, c1 used(%)';
	            if(isOverFileSystem){
	            	innerString += '<span style="font-size: 14px;"><br>'+ fileSystemCriteria +'% 초과</span>';
	            }
	            innerString +=  '</th>';
	            
	            innerString +=  '<th id="loadavgHeader" class="' + isOverLoadAvg + '" style="width: 134px;">LoadAvg';
	            if(isOverLoadAvg){
	            ...
	            if(serverList[num]['xvd'][0] >= fileSystemCriteria){
	            	issue.push('/xvda '+ fileSystemCriteria +'% 초과');
	            }
	            
	            if(serverList[num]['xvd'][1] >= fileSystemCriteria){
	            	issue.push('/xvdb '+ fileSystemCriteria +'% 초과');
	            }
	            
	            if(serverList[num]['xvd'][2] >= fileSystemCriteria){
	            	issue.push('/xvdc '+ fileSystemCriteria +'% 초과');
	            }
	            ...
	            if(isOverMemory){
	            	issue.push('Memory 90% 초과');
	            }
	            if(isOverDisk){
	            	issue.push('Disk 90% 초과');
	            }	
	            ...
	            resultArray.push(manager);//모니터링 담당자
